...
const AUFGABE_LABEL = {
  ...
  SCHEIBENSETZER: 'Scheibensetzer',
  ZEITNAHME_PROTOKOLL: 'Zeitnahme & Protokoll'
}
const STATUS_CLS = {
...
export default function HelferPage() {
  ...
  const [form, setForm] = useState({ name: '', aufgabe: '' })
  ...
  const handleAdd = async (e) => {
    ...
    setError('')
    try {
      await apiFetch('/helfer', { method: 'POST', body: { ...form, jaegerschaftId } })
      setForm({ name: '', aufgabe: '' })
      load()
    } catch (err) { setError(err.message) }
  }
          ...
          {AUFGABEN.map(a => {
            ...
            return (
              <div key={a} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
                <span className="text-sm text-gray-700">{AUFGABE_LABEL[a]}</span>
                {h
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5 mb-6">
        ...
        <form onSubmit={handleAdd} className="flex gap-3">
          <input className="flex-1 border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-jagd-500"
            value={form.name} onChange={e => setForm(f => ({...f, name: e.target.value}))} placeholder="Name des Helfers" required />
          <select className="border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-jagd-500"
            value={form.aufgabe} onChange={e => setForm(f => ({...f, aufgabe: e.target.value}))} required>
            <option value="">– Aufgabe –</option>
            {AUFGABEN.map(a => <option key={a} value={a}>{AUFGABE_LABEL[a]}</option>)}
          </select>
          <button type="submit" className="px-4 py-2 text-sm font-medium bg-jagd-500 text-white rounded-lg hover:bg-jagd-600 whitespace-nowrap">
            Hinzufügen
          </button>
        </form>
      </div>
          ...
          : helfer.map(h => (
            <div key={h.id} className="flex items-center gap-3 px-5 py-3 border-b border-gray-50 last:border-0">
              <div className="flex-1">
                ...
                <p className="text-xs text-gray-400">{AUFGABE_LABEL[h.aufgabe]}</p>
              </div>
              ...
              <button onClick={() => handleDelete(h.id)} className="text-xs text-gray-400 hover:text-red-600 px-2 py-1">Löschen</button>
            </div>
          ))
        }
      </div>
